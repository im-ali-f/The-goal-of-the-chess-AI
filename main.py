        ...
        if temp_main_piece[0] == "H" or  temp_main_piece[0] == "P" or  temp_main_piece[0] == "Q" or  temp_main_piece[0] == "R" or  temp_main_piece[0] == "B" :
            ...
            for main_asar in main_asar_koli:
                ...
                for kisher in black_king_kisher: #j i ksi ke kish karde
                    kisher_j=kisher[0]
                    kisher_i=kisher[1]
                    kisher_asar_koli=globals()[f"square_{kisher_j}{kisher_i}"].asarha #gereftan asarha ksi ke kish karde
...
def kish_va_mate():
    global piece_id_counter
    global piece_id_list
    global black_piece_attack
    global white_piece_attack
    global black_king_kisher
    global white_king_kisher
    global block_list
    global white_kish
    ...
    global black_king_move_list
    global white_king_move_list
    global black_king_kish_list
    global white_king_kish_list
    global black_king_position
    global white_king_position
    global wehave_white_king
    global wehave_black_king
    ...
    global apiece_can_uncheck_black
    global apiece_can_uncheck_white
    global black_friendly_list
    global last_black_friendly_can_move
    global white_friendly_list
    global last_white_friendly_can_move
    global last_black_piece_move
    global last_white_piece_move
    global natije_show_white
    global natije_show_black
            ...
            if len_black_king_move_list !=0:
                ...
                if temp_piece_len >0:
                    ...
                    if temp_friendly_move_len >0:
                        C = ""
                        temp_black = []
                        for last in last_black_piece_move:
                            if last not in temp_black:
                                temp_black.append(last)
                        for friend_who_canmove in temp_black:
                            temp_C=f"{friend_who_canmove[2][0]}   can uncheck king  by moving to  =>  {friend_who_canmove[0],friend_who_canmove[1]}\n"
                            C+=temp_C
                            black_mate=0
                    if temp_friendly_move_len == 0:
                        black_mate=1
            ...
            if len_white_king_move_list != 0:
                ...
                if temp_piece_len > 0:
                    ...
                    if temp_friendly_move_len > 0:
                        c=""
                        temp_white=[]
                        for last in last_white_piece_move:
                            if last not in temp_white:
                                temp_white.append(last)
                        for friend_who_canmove in temp_white:
                            temp_c=f"{friend_who_canmove[2][0]}   can uncheck king  by moving to  =>  {friend_who_canmove[0],friend_who_canmove[1]}\n"
                            c+=temp_c
                            white_mate = 0
                    if temp_friendly_move_len == 0:
    ...
    natije_show_white.place_forget()
    natije_show_black.place_forget()
    b_kish_count = 0
    if wehave_black_king == True:
        temp_b_king=globals()[f"square_{black_king_position[0]}{black_king_position[1]}"].piece
        for char in temp_b_king:
            if char == ")":
                b_kish_count+=1

    w_kish_count = 0
    if wehave_white_king ==True:
        temp_w_king = globals()[f"square_{white_king_position[0]}{white_king_position[1]}"].piece
        for char in temp_w_king:
            if char == "}":
                w_kish_count += 1
    ...
    natije_show_white.configure(bg="blue",font = myFont)
    natije_show_black = Message(root,text=y)
    natije_show_black.configure(bg="orange",font = myFont)



    natije_show_white.place(relx = 1, x =-2, y = 2, anchor = NE)
    natije_show_black.place(relx = 1, x =-2, y = 200, anchor = NE)
    ...
    for j in satr:
        for i in soton:
            globals()[f"square_{j}{i}"].satr=j
            globals()[f"square_{j}{i}"].soton=i
            globals()[f"square_{j}{i}"].piece="."
            globals()[f"square_{j}{i}"].asar=[]
            globals()[f"square_{j}{i}"].asarha=[]

    print("\n\n ##################################################")
    asargozari_btn.configure(state=ACTIVE)
    natije_btn.configure(state=DISABLED)
...
def asargozari_for_front():
    ...
    for p in piece_id_list:
        asargozari(p)
    asargozari_btn.configure(state=DISABLED)
    natije_btn.configure(state=ACTIVE)


# ((((((((((((()))))))))))))
root=Tk()
myFont = tkFont.Font(family="Times New Roman", size=15, weight="bold")
natije_show_white = Message(root)
natije_show_white.configure(bg="blue",font = myFont)
natije_show_black = Message(root)
natije_show_black.configure(bg="orange",font = myFont)
...
for sq in square_list:
    ...
    globals()[f"square_{j}{i}_show"].configure(padx=30,pady=30,fg="#F3120B",font = myFont)
    ...
    if temp_color==0 and counter_color<8:
        globals()[f"square_{j}{i}_show"].configure(bg="#EDE8E7",font = myFont)
        temp_color=1
        counter_color+=1
    elif temp_color==1 and counter_color<8:
        globals()[f"square_{j}{i}_show"].configure(bg="#2C2929",font = myFont)
        temp_color = 0
        counter_color += 1
